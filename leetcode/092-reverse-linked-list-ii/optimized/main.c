 ...
 * };
 *
 * Optimized: use a stack-based dummy node (no malloc/leak),
 * and clearer variable names (prev, tail, move).
 *
 * Head-insertion technique: after navigating to position left,
 * repeatedly take the node after tail and insert it right after prev.
 * This reverses the [left, right] segment in-place with one pass.
 */
